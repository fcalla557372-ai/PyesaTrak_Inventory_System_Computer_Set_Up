...
class StaffDashboardView(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.COLORS = {'primary': '#0076aa', 'secondary': '#38b6ff', 'black': '#000000', 'bg_gray': '#f2f2f2',
                       'white': '#FFFFFF', 'danger': '#D32F2F', 'warning': '#FF9800', 'purple': '#9C27B0'}
    ...
    def init_ui(self):
        ...
        self.sign_out_btn.setStyleSheet(
            f"QPushButton {{ background-color: #333333; color: white; border-radius: 25px; }} QPushButton:hover {{ background-color: {self.COLORS['danger']}; color: white; }}")
    ...
    def create_nav_btn(self, text):
        ...
        btn.setStyleSheet(
            f"QPushButton {{ background-color: transparent; color: #888888; text-align: left; padding-left: 20px; border: none; border-radius: 25px; }} QPushButton:hover {{ color: {self.COLORS['white']}; }}")
        return btn
    ...
    def update_button_styles(self, active_btn):
        ...
        for btn in buttons:
            if btn == active_btn:
                btn.setStyleSheet(
                    f"background-color: {self.COLORS['primary']}; color: white; text-align: left; padding-left: 20px; border: none; border-radius: 25px;")
            else:
                btn.setStyleSheet(
                    "background-color: transparent; color: #888888; text-align: left; padding-left: 20px; border: none; border-radius: 25px;")
    ...
    def create_dashboard_page(self):
        ...
        ref_btn.setStyleSheet(f"background-color: {self.COLORS['secondary']}; color: black; border-radius: 22px;")
        ...
        self.card_def = self.create_clickable_kpi(kpi_row, "DEFECTIVE", "0", self.COLORS['purple'])
        self.card_def['frame'].mousePressEvent = lambda e: self.kpi_defective_clicked.emit()
        self.card_def['frame'].setCursor(Qt.CursorShape.PointingHandCursor)
        self.lbl_def = self.card_def['label']
        ...
        self.activity_table.setStyleSheet(
            f"QTableWidget {{ border: none; background-color: white; color: black; font-family: 'Poppins'; }} QHeaderView::section {{ background-color: white; color: #888; font-weight: bold; border-bottom: 2px solid #eee; }} QTableWidget::item {{ padding: 8px; }}")
