# ADBoardView.py - FINAL CLEAN VERSION (No Borders)
...
class DashboardView(QWidget):
    ...
    def init_ui(self):
        ...
        app_title.setStyleSheet(f"color: {self.COLORS['white']}; margin-bottom: 30px;")
    ...
    def create_dashboard_page(self):
        """Dashboard with borderless, clean design"""
        page = QWidget()
        ...
        self.card_defect, self.lbl_defect = self.create_borderless_kpi(
            "Defective Items", "0", "Reported defects",
            self.COLORS['purple'], clickable=True,
            on_click=lambda: self.kpi_defective_clicked.emit()
        )
        kpi_grid.addWidget(self.card_defect, 0, 3)

        layout.addLayout(kpi_grid)

        # Bottom Section - NO BORDERS on titles
    ...
    def create_borderless_kpi(self, title, value, subtitle, status_color, clickable=False, on_click=None):
        """KPI card with NO BORDERS - completely clean"""
        card = QFrame()
        card.setFixedHeight(140)

        # COMPLETELY BORDERLESS - Only background color
        base_style = f"""
            QFrame {{
                background-color: {self.COLORS['white']}; 
                border: none;
